...
@app.route('/popravci_u_tijeku', methods=['GET', 'POST'])
def popravci_u_tijeku():
    registracija_filter = request.form.get('registracija_filter')

    if registracija_filter:
        popravci_u_tijeku = PopravakUTijeku.query.filter_by(registracija=registracija_filter).all()
    else:
        popravci_u_tijeku = PopravakUTijeku.query.all()
